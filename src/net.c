...

{
    ...
    EVP_PKEY *pkey = PEM_read_PrivateKey(dhparamfile, NULL, NULL, NULL);

    fclose(dhparamfile);


    if (!pkey || SSL_CTX_set0_tmp_dh_pkey(listener->ssl_ctx, pkey) != 1) {
...

{
	...
	EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new_id(EVP_PKEY_NONE, NULL);
	...
	OSSL_PROVIDER *provider = OSSL_PROVIDER_load(NULL, listener->tls_engine);

    if (!provider) {

        log__printf(NULL, MOSQ_LOG_ERR, "Error loading %s provider\n", listener->tls_engine);
	...
	if(listener->tls_keyform == mosq_k_engine){
		...
		if(listener->tls_engine_kpass_sha1){
			...
			if(!pctx){
				...
				log__printf(NULL, MOSQ_LOG_ERR, "Error creating PKEY context.");

				return MOSQ_ERR_TLS;

			}

			if(EVP_PKEY_CTX_ctrl_str(pctx, "ENGINE_SECRET_MODE", "ENGINE_SECRET_MODE_SHA") <= 0){

				log__printf(NULL, MOSQ_LOG_ERR, "Error setting ENGINE_SECRET_MODE to SHA.");
			...
			if(EVP_PKEY_CTX_ctrl_str(pctx, "ENGINE_PIN", listener->tls_engine_kpass_sha1) <= 0){

				log__printf(NULL, MOSQ_LOG_ERR, "Error setting ENGINE PIN.");
		...
		OSSL_STORE_CTX *store_ctx = OSSL_STORE_open(listener->keyfile, NULL, NULL, NULL, NULL);

		if (!store_ctx) {

			log__printf(NULL, MOSQ_LOG_ERR, "Error opening key file with provider.");
